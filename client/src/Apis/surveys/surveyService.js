...
export const getEodSurveys = async (
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/endofdaysurvey/getEndOfDaySurveys?page_no=${page_no}&no_of_surveys=${surveys_per_page}`
    );
...
export const getBaselineSurveys = async (
  ...
  // &age=${age}&pain_open_surgery=${pain_open_surgery}&pain_laparoscopic_surgery=${pain_laparoscopic_surgery}&pain_robotic_surgery=${pain_robotic_surgery}&pain_past_six_months=${pain_past_six_months}&pain_interfered_relations=${pain_interfered_relations}&pain_interfered_sleep=${pain_interfered_sleep}&height=${height}&gender=${gender}&handness=${handness}&glove_size=${glove_size}&userid=${userid}&surgical_procedures_day=${surgical_procedures_day}&exercise=${exercise}&primary_speciality=${primary_speciality}&years_open_surgery=${years_open_surgery}&years_laparoscopic_surgery=${years_laparoscopic_surgery}&years_robotic_surgery=${years_robotic_surgery}&most_common_procedures_a=${most_common_procedures_a}&most_common_procedures_b=${most_common_procedures_b}&most_common_procedures_c=${most_common_procedures_c}&createdAt=${createdAt}&updatedAt=${updatedAt}
  try {
    const response = await axios.get(
      `${BASE_URL}/baselinesurvey/getBaselineSurveys?page_no=${page_no}&no_of_surveys=${no_of_baseline}`
    );
    return response.data;
  } catch (error) {
    console.log(error);
