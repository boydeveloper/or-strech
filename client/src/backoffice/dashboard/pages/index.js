export { default as Overview } from "./overview/overview";
export { default as Users } from "./users/users";
export { default as UserActivities } from "./userActivities/userActivities";
export { default as AddUsers } from "./addUser/addUser";

export { default as ManageTags } from "./manageTags/manageTags";
export { default as AddVideos } from "./addVideos/addVideos";

export { default as ManageVideos } from "./manageVideos/manageVideos";
export { default as UpdateUser } from "./updateUser/updateUser";
export { default as UpdateTag } from "./updateTag/updateTag";
export { default as UpdateVideos } from "./updateVideos/updateVideos";
export { default as TagDetails } from "./tagDetails/tagDetails";
export { default as Profile } from "./profile/profile";
